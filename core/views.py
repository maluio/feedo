...
def feed_articles(request, feed_id):
    articles = Article.objects.filter(feed__id=feed_id, unread=True).order_by('-created_at')
...
def forward_to_next_feed(request):
    ...
    return HttpResponseRedirect(reverse('core:feed_articles', kwargs={'feed_id': feed.id}))
